...
class Granule(nn.Module):
    ...
    def forward(self, nonindi, indi):
        ...
        for i in range(b):
            ...
            for k in range(a):
                temp_indi_value = indi_value[i, k, :]
                temp_sample = torch.cat((temp_nonindi, temp_indi_value), dim=0)
                factual_indi[i, k, :] = temp_sample
            for j in range(b):
                temp_indi = indi[j]
                counter_sample = torch.cat((temp_nonindi, temp_indi), dim=0)
                counter_swap[i, j, :] = counter_sample

        factual_indi = factual_indi.view(b * a, -1)
        factual_indi = self.net(factual_indi)
        factual_indi = factual_indi.view(b, a, -1)
